...
-(void)dealloc{
	...
	[self.XMLSource release];
	[self.currentParsedCharacterData release];
	[self.currentEntry release];
	[self.parsedFeed release];	
...
//		self.currentEntry.source = (NSString*)[attributeDict objectForKey:@"gr:stream-id"];
		self.currentEntry.source = (NSString*)[attributeDict objectForKey:@"stream-id"];
	}
