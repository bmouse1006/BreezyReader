...
-(void)finishedDownloadingGRItem:(GRItem*)item{
	...
	GRSubModel* sub = [self downloadedSubscriptionForID:item.source];
...
-(void)dealloc{
	[self.downloaderQueue release];
	[self.downloaderIndex release];
	[self.itemDownloaderDictionary release];
