...
-(void)mainDownloadTask{
	...
	NSString* excludeLabel = [ATOM_PREFIXE_STATE_GOOGLE stringByAppendingString:ATOM_STATE_READ];
...
-(void)finishedDownloadingGRItem:(GRItem*)item{
	...
	@synchronized(_currentDownloaderPool){
		...
		GRSubModel* sub = [[GRDownloadManager shared] downloadedSubscriptionForID:item.source];
...
-(void)dealloc{
	...
	[self.currentDownloaderPool release];
	[self.operationQueue release];
	[self.downloadError release];
	[self.thread release];
